...
export class ObjectStorageService {
  ...
  async getUploadURL(fileName: string): Promise<{ uploadURL: string; filePath: string }> {
    const privateObjectDir = this.getPrivateObjectDir();
    const objectId = randomUUID();
    const sanitizedFileName = fileName.replace(/[^a-zA-Z0-9.-]/g, '_');
    const fullPath = `${privateObjectDir}/job-attachments/${objectId}-${sanitizedFileName}`;
  ...
  async getViewURL(filePath: string): Promise<string> {
    ...
    const viewURL = await signObjectURL({
      ...
      objectName,
      method: "GET",
      ttlSec: 3600, // 1 hour validity for viewing
    });
